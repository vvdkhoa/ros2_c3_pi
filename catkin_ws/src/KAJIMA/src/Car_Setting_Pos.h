...
public:
	CarPosCalsulate(double WHEEL_plus_per_deg,double Wheel_plus_per_meter, double CAR_Wheel_to_Wheel_L_meter) {
		Wheel_plus_per_deg = WHEEL_plus_per_deg;
		Wheel_plus_per_mm = Wheel_plus_per_meter / 1000.0;
		CAR_WELL_to_Weel_L_mm = CAR_Wheel_to_Wheel_L_meter * 1000;
	};
	~CarPosCalsulate() {};
...
private:
	...
	double Wheel_plus_per_deg;
	double Wheel_plus_per_mm;
	double CAR_WELL_to_Weel_L_mm;
private:
	int32 Speed_L_Pre;
	int32 Speed_R_Pre;
	int ERROR_Code_SAVE;
...
public:
	void initial(int StepL, int StepR) {
		...
		MotorStepT = Degree * Wheel_plus_per_deg;
		...
		TXT_FUNC_TAG(TxtTag,TxtOutBuffer,"initial (%d,%d)\n", MotorStepL, MotorStepR);
	...
	void initial(int StepL, int StepR, double x, double y, double degree) {
		...
		TXT_FUNC_TAG(TxtTag,TxtOutBuffer,"initial (%d,%d),(%.1f,%.1f,%.1f)\n", MotorStepL, MotorStepR, X, Y, Degree);
	};
...
*/
		...
		if (ERROR_Code) {
			if (ERROR_Code_SAVE != ERROR_Code) {
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "GetPos ERROR CODE = %d \n", ERROR_Code);
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "ERROR DATA = [Motor1 = %d],[Motor_2 = %d]\n", StepL, StepR);
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "[M_1_Pre = %d],[M_2_Pre = %d]\n", MotorStepL, MotorStepR);
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "[M_diff = %d],[M_diff = %d]\n", steptmpl, steptmpr);
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "[SPD_1 = %d],[SPD_2 = %d]\n",  SpeedL, SpeedR);
				TXT_FUNC_TAG(TxtTag, TxtOutBuffer, "[SPD_Pre_1 = %d],[SPD_Pre_2 = %d] \n", Speed_L_Pre, Speed_R_Pre);
		...
		if ((steptmpl || steptmpr) || (Speed_L_F || Speed_R_F)) {
			...
			//TXT_FUNC_TAG(TxtTag,TxtOutBuffer,"L = %f , R = %f, SpeedRate = %f_%f,%f __ ", Speed_L_F, Speed_R_F,SpeedRate, abs(SpeedRate), fabs(SpeedRate));

			if (fabs(SpeedRate) < 0.05) {
				//TXT_FUNC_TAG(TxtTag,TxtOutBuffer,"SpeedRate < 0.05\n");
				if (steptmpl > 0 && steptmpr < 0) {  //FWD
					changerate = Wheel_plus_per_mm;
					...
					MotorStepT -= steptmpl;
					Degree = (MotorStepT / Wheel_plus_per_deg);
					AllRangeDegree = Degree;
				}
				...
				if (steptmpl > 0 && steptmpr < 0) {  //FWD
					...
					if (abs(steptmpl) > abs(steptmpr)) {
						...
						r1 = CAR_WELL_to_Weel_L_mm / (SpeedRate - 1);
						L = fabs(steptmpl / Wheel_plus_per_mm);
						sitaR = -L / (r1 + CAR_WELL_to_Weel_L_mm);
					}
					else {
						SpeedRate = fabs((double)Speed_R_F / (double)Speed_L_F);
						r1 = CAR_WELL_to_Weel_L_mm / (SpeedRate - 1);
						L = fabs(steptmpr / Wheel_plus_per_mm);
						sitaR = L / (r1 + CAR_WELL_to_Weel_L_mm);
					}
					sitaD = 0.5 * sitaR * 180 / M_PI; // RAD to DEG

					MotorStepT += sitaD * Wheel_plus_per_deg;
					Degree = (MotorStepT / Wheel_plus_per_deg);
					SelfPos.Degree += sitaD;
					AllRangeDegree = Degree;

					rc = r1 + (0.5*CAR_WELL_to_Weel_L_mm);
					move_L = 2 * rc* sin(fabs(sitaR)*0.5);
					X = X + (move_L *cos(Degree*M_PI / 180.0));
					Y = Y + (move_L *sin(Degree*M_PI / 180.0));
					MotorStepF += move_L / Wheel_plus_per_mm;
	...
	bool Pos_2_RootCalculate(int step = 0) {
		...
		for (int i = 0; i < MotorCmd.size(); i++) {
			TXT_FUNC_TAG(TxtTag,TxtOutBuffer,"act[%d]_[%c][%f]\n", i, MotorCmd.at(i).Action, MotorCmd.at(i).Value);
		}
	}

	int Distance_2_Motor_Step(char type, double Distance) {
		if (type == 'F' || type == 'B') {
			Distance = Distance * Wheel_plus_per_mm;

		}
		else if (type == 'L' || type == 'R') {
			Distance = Distance * Wheel_plus_per_deg;
		}
		return Distance;
	}
